...
jQuery( document ).ready(function() {
  ...
  //SHOW MODAL RESOURCE LIST
	jQuery(document).on('click','.lp-resource-selector-button',function(e){
		e.preventDefault ? e.preventDefault() : e.returnValue = false;
    tmp_this = jQuery(this);
		jQuery('.lp-resource-selector-tag-p').removeClass('checked');
    var prv = jQuery(this).siblings('input[name="oer_lp_primary_resources[resource][]"]').val();
    var rad = jQuery('input[name="lp-resource-selector-rad"][value="'+prv+'"]');
		rad.attr('checked', true);
		rad.parent().addClass('checked');
		jQuery('.lp-resource-selector-overlay').removeClass('fadeOut').addClass('fadeIn').css('visibility', 'visible');
		jQuery('input[name="lp-resource-selector-criteria"]').val('');
		jQuery('#lp_resource_selector-dynahide').remove();
    jQuery('.lp-resource-selector-criteria').focus();
	});
	
  //PUT A CHECK ON AN ITEM IN THE LIST
	jQuery(document).on('change','input[name="lp-resource-selector-rad"]', function(){
		jQuery('.lp-resource-selector-tag-p').removeClass('checked');
		jQuery(this).parent().addClass('checked');
	});
	
  //CLOSE THE MODAL RESOURCE LIST
	jQuery(document).on('click','.lp-resource-selector-search-close', function(e){
		e.preventDefault ? e.preventDefault() : e.returnValue = false;
		jQuery('.lp-resource-selector-overlay').removeClass('fadeIn').addClass('fadeOut');
		setTimeout(function(){ jQuery('.lp-resource-selector-overlay').css('visibility', 'hidden') }, 500);
	});
	
  //INPUT SEARCH CRITERIA
	jQuery(document).on('input','input[name="lp-resource-selector-criteria"]', function(e) {
		var searchTerm = jQuery.trim(this.value).toLowerCase();
		jQuery('#lp_resource_selector-dynahide').remove();
		if(searchTerm > ''){
		  jQuery('#lp_resource_selector-dynahide').remove();
		  css = '.lp-resource-selector-search-result .lp-resource-selector-tag-p:not([data-search-term*="'+searchTerm+'"]){display:none;}'
		  style = jQuery('<style type="text/css" id="lp_resource_selector-dynahide">').text(css)
