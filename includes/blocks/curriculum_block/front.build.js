jQuery( document ).ready(function() {
    ...
    // DISPLAY BOX
    jQuery(document).on('click','.lp-cur-blk-topbar-display-text>a',function(e){
      e.preventDefault ? e.preventDefault() : e.returnValue = false;
      var target = jQuery(this).parent().siblings('ul');
      lpCurTogleOptions(target); 
    })  
    jQuery(document).on('keydown','.lp-cur-blk-topbar-display-text>a',function(e){
    ...
    // SORT BOX
    jQuery(document).on('click','.lp-cur-blk-topbar-sort-text>a',function(e){
      e.preventDefault ? e.preventDefault() : e.returnValue = false;
      var target = jQuery(this).parent().siblings('ul');
      lpCurTogleOptions(target); 
    })
    jQuery(document).on('keydown','.lp-cur-blk-topbar-sort-text>a',function(e){
      var keycode = (e.keyCode ? e.keyCode : e.which);
      if(keycode != '9') {// other than tab
        e.preventDefault ? e.preventDefault() : e.returnValue = false;
        var target = jQuery(this).parent().siblings('ul');
    ...
    // SUB MENU EVENTS
    jQuery(document).on('click','.lp-cur-blk-topbar-display-option li a',function(e){
        e.preventDefault ? e.preventDefault() : e.returnValue = false;
        var bid = jQuery(this).parents('.lp-cur-blk-main').attr('blockid');
        var val = jQuery(this).attr('ret');      
        lpCurSaveToLocalAttribute("postsPerPage-"+bid, val);
        var target = jQuery(this).parent();
        resetSelection(target);
    });  
    jQuery(document).on('keydown','.lp-cur-blk-topbar-display-option li a',function(e){
        e.preventDefault ? e.preventDefault() : e.returnValue = false;
        var keycode = (e.keyCode ? e.keyCode : e.which);
        var bid = jQuery(this).parents('.lp-cur-blk-main').attr('blockid');
        var val = jQuery(this).attr('ret');
    ...
    jQuery(document).on('click','.lp-cur-blk-topbar-sort-option li a',function(e){
        e.preventDefault ? e.preventDefault() : e.returnValue = false;
        var val = jQuery(this).attr('ret');
        var bid = jQuery(this).parents('.lp-cur-blk-main').attr('blockid');
    ...
    jQuery(document).on('keydown','.lp-cur-blk-topbar-sort-option li a',function(e){
        e.preventDefault ? e.preventDefault() : e.returnValue = false;
        var keycode = (e.keyCode ? e.keyCode : e.which);
        var bid = jQuery(this).parents('.lp-cur-blk-main').attr('blockid');
        var val = jQuery(this).attr('ret');
...
function resetSelection(target){
  ...
  var instanceparent = target.parents('.lp-cur-blk-main');
...
function lpCurShowHide(target){
  if(target.is(':visible')){
    ...
    jQuery('.lp-cur-blk-topbar-option').hide(300);
    target.show(300);  
  } 
}