...
function oercurr_cb_render_posts_block($attributes, $ajx=false){
    ...
    foreach($posts as $post){
        $_content .= '<div class="oercurr-blk-row">';
            ...
            $_content .= '<div class="oercurr-blk-right">';
                $_content .= '<div class="ttl"><a href="'.esc_url(get_post_permalink($post->ID)).'">'.esc_html($post->post_title).'</a></div>';
                $_content .= '<div class="oercurr-postmeta">';
                    if(count($post->oer_curriculum_grades)>1){
                        $_content .= '<span class="oercurr-postmeta-grades"><strong>Grades:</strong> '. esc_html($post->oer_curriculum_grades[0]).'-'.esc_html($post->oer_curriculum_grades[count($post->oer_curriculum_grades)-1]).'</span>';
                    }else{
                        if($post->oer_curriculum_grades[0] != ''){
                                $_content .= '<span class="oercurr-postmeta-grades"><strong>Grade:</strong> '. esc_html($post->oer_curriculum_grades[0]).'</span>';
                        }
                    }
                $_content .= '</div>';                    
                if(trim($post->post_content," ") != ''){
                    $_content .= '<div class="desc">'.substr(esc_html(wp_strip_all_tags($post->post_content)),0,180).' ...</div>';
                }            
                $_arr_tag = get_the_tags($post->ID);
                $_content .= '<div class="oercurr-tags tagcloud">';
                if(!empty($_arr_tag)){
                    foreach($_arr_tag as $key => $tag) {
                        $_content .= '<span><a href="'.esc_url(get_home_url().'/tag/'.esc_html($tag->slug)).'" alt="" class="button">'.esc_html($tag->name).'</a></span>';
                    }
                }
                $_content .= '</div">';                    
            $_content .= '</div>';
    ...
    */
    ...
    /*
    $_wrapper .= '<div class="oercurr-blk-main" blockid="'.esc_html($bid).'">';
        $_wrapper .= '<script>';
                $_wrapper .= 'localStorage.setItem("selectedCategory-'.esc_html($bid).'", "'.esc_html($attributes['selectedCategory']).'");';
                $_wrapper .= 'localStorage.setItem("postsPerPage-'.esc_html($bid).'", "'.esc_html($attributes['postsPerPage']).'");';
                $_wrapper .= 'localStorage.setItem("sortBy-'.esc_html($bid).'", "'.esc_html($attributes['sortBy']).'");';
        $_wrapper .= '</script>';
        $_wrapper .= '<div class="oercurr-blk-topbar">';    
            $_wrapper .= '<div class="oercurr-blk-topbar-left">';
                $_wrapper .= '<span>Browse All '.esc_html($_count).' Curriculums</span>';
            $_wrapper .= '</div>';
            $_wrapper .= '<div class="oercurr-blk-topbar-right">';    
                    $_wrapper .= '<div class="oercurr-blk-topbar-display-box">';
                        $_wrapper .= '<div class="oercurr-blk-topbar-display-text"><span>Show '.esc_html($attributes['postsPerPage']).'</span><a href="#"><i class="fa fa-th-list" aria-hidden="true"></i></a></div>';
                        $_wrapper .= '<ul class="oercurr-blk-topbar-display-option oercurr-blk-topbar-option" style="display:none;">';    
                                    for ($i=5; $i <=30; $i+=5){ 
                                         if($i == $attributes['postsPerPage']){
                                             $_wrapper .= '<li class="selected"><a href="#" ret="'.esc_html($i).'">'.esc_html($i).'</a></li>';
                                         }else{
                                             $_wrapper .= '<li><a href="#" ret="'.esc_html($i).'">'.esc_html($i).'</a></li>';
                                         }
                                    }
                        $_wrapper .= '</ul>';
                    $_wrapper .= '</div>';                    
                    $_wrapper .= '<div class="oercurr-blk-topbar-sort-box">';
                        $_wrapper .= '<div class="oercurr-blk-topbar-sort-text"><span>Sort by: '.esc_html($attributes['sortBy']).'</span><a href="#"><i class="fa fa-sort" aria-hidden="true"></i></a></div>';
                        $_wrapper .= '<ul class="oercurr-blk-topbar-sort-option oercurr-blk-topbar-option" style="display:none;">';
                                    ...
                                    $_wrapper .= '<li '.esc_html($_sel).'><a href="#" ret="date">Date Added</a></li>';
                                    $_sel = ($attributes['sortBy'] == 'modified')? 'class="selected"':'';
                                    $_wrapper .= '<li '.esc_html($_sel).'><a href="#" ret="modified">Date Updated</a></li>';
                                    $_sel = ($attributes['sortBy'] == 'title')? 'class="selected"':'';
                                    $_wrapper .= '<li '.esc_html($_sel).'><a href="#" ret="title">Title a-z</a></li>';
                        $_wrapper .= '</ul>';
                    $_wrapper .= '</div>';                    
            $_wrapper .= '</div>';
        $_wrapper .= '</div>';
