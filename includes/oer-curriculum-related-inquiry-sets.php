...
$inquiry = get_post_meta($inquiryset_post->ID,'oer_curriculum_related_inquiry_set');
            ...
            <div class="panel panel-default oer-curriculum-related-inquiry-wrapper oer-related-inquiry-sets" id="oer-related-inquiry-sets-group">
                <div class="panel-body">
                    ...
                    for ($i=1;$i<=3;$i++){
                        ...
                        $related_inquiry_set = (get_option('oer_curriculum_related_inquiry_set_'.$i.'_label'))?true:false;
                        $related_inquiry_set_enabled = (get_option('oer_curriculum_related_inquiry_set_'.$i.'_enabled'))?true:false;
                        if (($related_inquiry_set && $related_inquiry_set_enabled) || !$related_inquiry_set) {
                            $label = oer_curriculum_get_field_label('oer_curriculum_related_inquiry_set_'.$i);
                    ?>
                    <div class="form-group">
                        <label for="relatedInquirySet<?php echo $i; ?>"><?php echo $label; ?></label>
                        <select name="oer_curriculum_related_inquiry_set[]" id="relatedInquirySet<?php echo $i; ?>" class="form-control">
